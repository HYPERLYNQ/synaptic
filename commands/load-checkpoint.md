...
Call the `context_load` MCP tool with `name` equal to the argument that follows `/load-checkpoint`.
...
If `checkpoint == null` and `candidates.length > 0`, list the candidates and ask the user to run `/load-checkpoint <exact-name>`.

If `checkpoint == null` and `candidates.length == 0`, reply: "No checkpoint matched `<name>`. Try `/list-checkpoints` to list recent ones."
